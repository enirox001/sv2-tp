...
{
    if (!backend || !backend->MarkDisconnected()) return;
...
void Sv2TemplateProvider::ThreadSv2ClientHandler(size_t client_id)
{
    try {
        ...
        while (!m_flag_interrupt_sv2) {
            ...
            if (!set_wait_next_in_progress(true)) {
                // Backend replacement clears generation-bound cache entries.
                // Drop local proxies and wait for the replacement generation.
                backend.reset();
                block_template.reset();
                template_backend.reset();
                current_template_id = 0;
